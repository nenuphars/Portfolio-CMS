...
function PublishButton({ postId }: Props) {
  ...
  async function handlePublish(e: React.MouseEvent<HTMLButtonElement>) {
    e.preventDefault();
    try {
      const response = await publishPost(postId, token!);
      console.log("Publish response:", response);
      router.refresh(); // Refresh the page to reflect the updated post status
    } catch (err) {
      console.log(err);
    }
  }
  ...
  return (
    ...
    >
      <div className="flex flex-row w-auto items-end justify-center gap-1">
        <h4 className="text-xl leading-none">Publish</h4>
        <svg
